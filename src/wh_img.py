...
def title_from_keyword(keyword):
    ...
    if len(search_results) > 0:
        return resize_title(
            "How To "
            + search_results[random.randrange(0, len(search_results))]["title"]
        )
    ...
    if not image_list:
        ...
        for i in image_list:
            im_data = requests.get(
                api + "query&titles=File:" + i + "&prop=imageinfo&iiprop=url"
            )
            ...
            for key in pages.keys():
                ...
                if (
                    image_url
                    != "https://www.wikihow.com/images/7/78/Incomplete_856.gif"
                ):
...
def print_tutorial(titleok):
    ...
    para = textwrap.wrap(titleok, width=45)

    current_h, pad = 5, 10
    for line in para:
        w, h = draw.textsize(line, font=font)
        draw.text(((1600 - w) / 2, current_h), line, font=font, fill="#000000")
        current_h += h + pad
